...
\end{proof}
Поговорим чуть больше про $\varnothing$. Почему мы решили что оно такое одно в природе, ведь почему нельзя сказать что пустота бывает разной? Для ответа на этот вопрос сначала нужно сказать что значит, что множества разные или одинаковы.
\begin{definition}
...
\end{definition}
Теперь можно ответить на изначальный вопрос.

\begin{statement}
Пустое множество -- единственно.
\end{statement}
\begin{proof}
Предположим противное. То есть нашлись два разных пустых множеств $\varnothing_1$ и $\varnothing_2$, причем $\varnothing_1\neq\varnothing_2$.
Если оба этих множеств пусты, то по определению \ref{def:varnothing}
\[\forall x\ (x\notin\varnothing_1)\text{ и }\forall y\ (y\notin\varnothing_2)\]
\end{proof}
...
\end{cases}\]
Если $(A\setminus B)=\varnothing$, то $\forall x\ (x\in A\Rightarrow x\in B)$, иначе вычитание было бы не пусто. Аналогично, если $(B\setminus A)=\varnothing$, то $\forall x\ (x\in B\Rightarrow x\in A)$. Совмещаем эти два утверждения и получаем что
...
\medskip\noindent\textbf{6.}
Докажите $A=B\iff (A\subset B\text{ и }B\subset A)$.\\
Проведите аналогию с\\ $a=b\iff (a\leq b\text{ и }a\geq b)$.
